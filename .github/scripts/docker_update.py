...
def find_changed_files(specified_dir=None):
    ...
    if specified_dir:
        ...
        changed_files = [item.a_path for item in repo.git.diff(diff_target, name_only=True).split('\n') if item]
...
def update_dockerhub_descriptions(affected_dirs):
    ...
    for directory in affected_dirs:
        ...
        if not os.path.isfile(readme_path):
            logger.info(f"Warning: {readme_path} not found, skipping full description update")
            full_desc = ""
        else:
        ...
        if not os.path.isfile(dockerfile_latest):
            logger.info(f"Warning: {dockerfile_latest} not found, skipping short description update")
            short_desc = ""
        else:
