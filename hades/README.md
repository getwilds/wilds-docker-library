...
- The tag is pinned to both a HADES version and a specific Databricks Runtime version (`Dockerfile_<hades-version>dbx`). Match it to your cluster's DBR version, since a mismatch between the container's R stack and the runtime host is unsupported by Databricks.
