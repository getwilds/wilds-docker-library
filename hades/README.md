...
- **Explicit `xz-utils` on `latest`/`1.20.0`**: `duckdb`'s source package unpacks a bundled `duckdb.tar.xz` via the `xz` CLI at configure time, not via any R package's `liblzma`/`libarchive` bindings. The Databricks runtime base ships `xz-utils` preinstalled (undeclared as an explicit dependency in earlier versions of this Dockerfile); `rocker/r-ver` does not, and omitting it fails `duckdb`'s configure step with `xz: Cannot exec: No such file or directory` — which in turn fails the whole restore, since `Hades` and several other lockfile packages depend on `duckdb`.
